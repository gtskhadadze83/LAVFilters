...
configure() (
  ...
  if [ "${arch}" == "x86_64" ]; then
    ...
    OPTIONS="--arch=x86_64 --cross-prefix=${cross_prefix} --target-os=mingw64 --pkg-config=pkg-config --enable-shared --disable-static --build-suffix=-lav --disable-muxers --disable-encoders"
